...
def fill_3D_array_slide(filename,array,threshold,dist,style,slide,axis,degree):
    ...
    try:
        for fname in images:
            ...
            if style == 0:
                _, gray = cv2.threshold(h_img, threshold, 255,threshold_mode )
            elif style == 1:
                _, gray = cv2.threshold(s_img, threshold, 255,threshold_mode )
            else:
                _, gray = cv2.threshold(v_img, threshold, 255,threshold_mode )
